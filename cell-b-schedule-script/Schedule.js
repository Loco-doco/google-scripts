...
function update_schedule_sheet(dateArray, scheduleArray) {
  ...
  const result = get_schedule_process_per_date(dateArray, scheduleArray)
 ...
 */
function get_schedule_process_per_date(dateArray, scheduleArray){
...
function get_schedule_row(values){
  ...
  const row = dateArray.reduce((acc,date)=>{
    ...
    else if(date >= minDate && date <= maxDate){ //n일차 표기
      acc.push(workProgress[i])
      i++
    }
    else {acc.push(0)}
