class Validation {
  ...
  */
  change_key_format(words, ranKeyCount, type = "PREVIEW") {
    const result = words.reduce((acc, val, i) => {
      ...
      switch (val[1]) {
         ...
         */
        case 2:
          [ranKeyCount, val[0]] = change_key_format_ran_err(
            i,
            2,
            this.patternRanKey,
            ranKeyCount,
  ...
  check_duplicated_keys(words, publishedWords) {
    ...
    // 입력한 것들 내에서 검증
    keyList.some((val) => {
      if (keyList.indexOf(val) !== keyList.lastIndexOf(val))
        throw new Error(
          `입력할 키 들 사이에 중복된 키 값이 존재합니다. \\n\\n<해당 스트링> \\n key : ${val}`
        );
    });
    ...
    words.reduce((acc, val1, i) => {
      publishedWords.some((val2) => {
        if (val2.includes(val1[0]))
          throw new Error(
            `이미 publish 된 키랑 중복된 키 값이 존재합니다. \\n\\n<해당 스트링> \\n row: ${
              i + 2
            } \\n key: ${val1[0]}`
          );
      });
  ...
  check_value_valid(val) {
    if (
      ...
      throw new Error(
        `빈 공백으로 시작하는 문자열이 존재합니다. \\n\\n<해당 스트링> \\n key : ${val[0]} \\n value : ${val[2]}`
      );
    if (
      String(val[2]).match(this.patternDoubleQuotes) ||
      String(val[3]).match(this.patternDoubleQuotes)
    )
      throw new Error(
        `쌍 따옴표 앞에는 반드시 백 슬래시를 넣어주세요. \\n\\n<해당 스트링> \\n key : ${val[0]} \\n value : ${val[2]}`
      );
    if (val[2].endsWith("\n") || val[3].endsWith("\n"))
      throw new Error(
        `개행 처리(엔터)로 끝나는 단어가 존재합니다. 마지막 엔터 처리를 지워주세요 \\n\\n <해당 스트링> \\n key : ${val[0]} \\n value : ${val[2]}`
      );
  }
  ...
  check_value_valid_single(val) {
    if (String(val).match(this.patternStartWithBlank))
      throw new Error(`빈 공백으로 시작하는 문자열이 존재합니다.`);
    if (String(val).match(this.patternDoubleQuotes))
      throw new Error(`쌍 따옴표 앞에는 반드시 백 슬래시를 넣어주세요.`);
    if (val.endsWith("\n"))
      throw new Error(
        `개행 처리(엔터)로 끝나는 단어가 존재합니다. 마지막 엔터 처리를 지워주세요`
      );
