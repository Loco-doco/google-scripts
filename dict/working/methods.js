...
function do_insert(){
  ...
  try{
    publish_words(isIncludeProdPublish)
...
/**
 * 각 환경별 시트에 publish 하기
 */
function publish_words(isIncludeProdPublish){
  if(isIncludeProdPublish){
    const dev = new BulkInsert("dev");
    const prod = new BulkInsert("prod");
    const devApprovedWords = dev.valid_procedure(dev.targetWords)
    const prodApprovedWords = prod.valid_procedure(prod.targetWords)
    dev.insert_words_to_published_sheet(devApprovedWords)
    prod.insert_words_to_published_sheet(prodApprovedWords)
    dev.clear_working_sheet()
  } else {
    const dev = new BulkInsert("dev");
    const devApprovedWords = dev.valid_procedure(dev.targetWords)
    dev.insert_words_to_published_sheet(devApprovedWords)
