class Translate extends Global {
  ...
  do_publish(targetSheet) {
    ...
    switch (targetSheet.rowColInfo.type) {
      ...
      case this.sheetRowColInfo.Terms.type:
        this.working_term_transaction(
          targetSheet.sheet,
          targetSheet.rowColInfo
        );
        break;

      case this.sheetRowColInfo.Partial1.type:
        this.working_partial_transaction(
          targetSheet,
          targetSheet.sheet,
          targetSheet.rowColInfo,
          targetWords
  ...
  push_words_to_published_sheet(targetWords) {
    ...
    const insertRowArr = targetWords.reduce((acc, val) => {
      insertIdx =
        Global.index_of_2d_array(publish.publishedKeyList, val[0])[0] + 1;
      acc.push(insertIdx);
      return acc;
    }, []);

    if (insertRowArr.length === 0)
      throw new Error(`배포할 단어가 존재하지 않습니다.`);

    const datetime = new Date();

    insertRowArr.reduce((acc, val, i) => {
      publish.p_wordSheet
        .getRange(val + 1, publish.p_col_lan_value, 1, 3)
        .setValues([[targetWords[i][4], datetime, this.userEmail]]);
    }, "");
  }
  ...
  reset_selected_keys(targetSheet, targetWords) {
    ...
    const resetValues = this.get_remain_dropdown_keys(
      prevSelectedKeys,
      targetWords
    );
    ...
    if (resetValues[0])
      sheet
        .getRange(
          rowColInfo.selectedKeyStartRow,
          rowColInfo.selectedKeyStartCol,
          resetValues.length,
          1
        )
  ...
  get_prev_selected_keys(sheet, rowColInfo) {
    const keyList = sheet
      .getRange(
        rowColInfo.selectedKeyStartRow,
        rowColInfo.selectedKeyStartCol,
        rowColInfo.selectedKeyRowRange,
        1
      )
      .getValues();
